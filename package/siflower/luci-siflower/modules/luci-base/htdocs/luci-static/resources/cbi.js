...
var cbi_validators = {
	...
	'or': function()
	{
		for (var i = 0; i < arguments.length; i += 2)
		{
			...
			/* start modified by ivan 2015.8.17 make function ".apply" compatible with all browsers */
			...
			if (arguments[i+1] == null)
			{
				...
				else if (arguments[i].apply(this, sub_arguments))
				{
					return true;
				}
			}
			/* end modified by ivan 2015.8.17 make function ".apply" compatible with all browsers */
...
function cbi_d_update(x) {
	var state = false;
	var obj = document.getElementById(x);

	if (obj)
	  if (obj.options[obj.selectedIndex].value == 'sta-wds')
		confirm('Before set to Client(WDS) mode, you should confirm following things.' + '\n' +
					'1、your IP of LAN should be different to the router you are connect to.' + '\n' +
					'2、WDS Server, Router, WDS Client should be in the same network segment.' + '\n' +
					'3、The WDS Server should be in Access Point(WDS) mode.' + '\n' +
					'4、If connect to wds server fail, you should configure wds client to static IP. So that you can still manage the router.')
...
function cbi_validate_form(form, errmsg)
{
	...
	if( form.cbi_validators )
	{
		...
		/* start modified by ivan 2015.8.17 inspect some options in "LAN" webpage, especially "IPv4 address" */
		for( var j = 1; j < 10; j++ )
		{
			if( form[j].id == 'cbid.network.lan.netmask' )
			{
				if( form[j].value == '')
				{
					alert('IPv4 netmask can not be null!');
					return false;
				}
			}

			if( form[j].id == 'cbid.network.lan.ipaddr' )
			{
				if( form[j].value == '')
				{
					alert('IPv4 address can not be null!');
					return false;
				}

				else
				{
					var ip_string = form[j].value.split(".");
					var _ip = parseInt(ip_string[3]);

					for( var k = 31; k < 40; k++ )
					{
						if( form[k].id == 'cbid.dhcp.lan.start' )
						{
							if( form[k].value == '' )
							{
								var _start = -1;
							...
							{
								var _start = parseInt(form[k].value);
							}
						}

						if( form[k].id == 'cbid.dhcp.lan.limit' )
						{
							if( form[k].value == '')
							{
								var _limit = -1;
							...
							{
								var _limit = parseInt(form[k].value);
							}
						}
					}
					...
					{
						alert('Start and Limit can not be null!');
						return false;
					}
					else
					{
						var _end = _start + _limit - 1;
						if( _start > 255 )
						{
							alert('Start must be less than 256!');
							return false;
						}
						else if( _end > 255 )
						{
							alert('Limit may be too big!');
							return false;
						}
						else if( _ip >= _start && _ip <= _end )
						{
							alert('IPv4 address\'s end should not be in ' + _start + ' ~ ' + _end + '!');
							return false;
						}
						else if( form[j].value != form[j].defaultValue)
						{
							if(confirm('IPv4 address has been changed, please refresh your networt to get the new ipaddr to visit the new web page ' + form[j].value + '!'))
							{
								return true;
							}
							else
		...
		/* end modified by ivan 2015.8.17 inspect some options in "LAN" webpage, especially "IPv4 address" */
	...
	if (field && vstack && typeof vstack[0] == "function")
	{
		var validator = function()
		{
			...
			if( field.form )
			{
				...
				{
					var sub_vstack = []
					if (!(((value.length == 0) && optional) || vstack[0].apply(value, sub_vstack)))
					{
						// invalid
						field.className += ' cbi-input-invalid';
						return false;
