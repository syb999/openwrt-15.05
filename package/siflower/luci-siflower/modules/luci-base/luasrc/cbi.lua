...
function send_changes(_changes)
    nixio.syslog("crit", "---------sned_changes : changes = ".._changes.."----------")
    ...
    local send_buf = "UCIM -data ".._changes
...
--add by kevin@siflower, 2015/09/28
--check the wifi disable/enable
local function check_wifi_enable(section_name)
    local wifi = _uci_real:get_all("wireless", section_name)
...
--add by kevin@siflower, 2015/09/28
--new a function to fetch the valuable changes and write it to uci

local function changes_fetch(changes)
    local pure_wifi_info_config = nil
    local pure_basic_setting_config = nil
    local B5G_cfg = nil
    local B2G_cfg = nil
    local device_5G = nil
    local device_2G = nil
    if changes.wireless then
        ...
        _uci_real:foreach("wireless", "wifi-iface",
            function(s)
                if s.device == device_5G then
                    B5G_cfg = s[".name"]
                elseif s.device == device_2G then
                    B2G_cfg = s[".name"]
                end
            end
        )
        pure_wifi_info_config = "wifi_info"
        local wl_changes = changes.wireless
        for key,val in pairs(wl_changes) do
            if key == device_5G then
                _uci_real:set(pure_wifi_info_config, "wifi5G", "device", key)
                if val.channel then _uci_real:set(pure_wifi_info_config, "wifi5G", "channel", val.channel) end
                if val.radio then
                    _uci_real:set(pure_wifi_info_config, "wifi5G", "enable", val.radio)

                    if val.radio == "1" then
                        _uci_real:delete("wireless", B5G_cfg, "disabled")
                    elseif val.radio == "0" then
                        _uci_real:set("wireless", B5G_cfg, "disabled", "1")
                    end
                end
            end
            if key == B5G_cfg then
                if val.ssid then _uci_real:set(pure_wifi_info_config, "wifi5G", "ssid", val.ssid) end
                if val.key then _uci_real:set(pure_wifi_info_config, "wifi5G", "password", val.key) end
                if val.encryption then _uci_real:set(pure_wifi_info_config, "wifi5G", "encryption", val.encryption)  end
            end
            if key == device_2G then
                _uci_real:set(pure_wifi_info_config, "wifi2_4G", "device", key)
                if val.channel then _uci_real:set(pure_wifi_info_config, "wifi2_4G", "channel", val.channel) end
                if val.radio then
                    _uci_real:set(pure_wifi_info_config, "wifi2_4G", "enable", val.radio)

                    if val.radio == "1" then
                        _uci_real:delete("wireless", B2G_cfg, "disabled")
                    elseif val.radio == "0" then
                        _uci_real:set("wireless", B2G_cfg, "disabled", "1")
                    end
                end
            end
            if key == B2G_cfg then
                if val.ssid then _uci_real:set(pure_wifi_info_config, "wifi2_4G", "ssid", val.ssid) end
                if val.key then _uci_real:set(pure_wifi_info_config, "wifi2_4G", "password", val.key) end
                if val.encryption then _uci_real:set(pure_wifi_info_config, "wifi2_4G", "encryption", val.encryption)  end
            end
    ...
    if changes.network then
        pure_basic_setting_config = "basic_setting"
        if changes.network.wan then
            if changes.network.wan.proto then
                if changes.network.wan.proto == "dhcp" then
                    _uci_real:set(pure_basic_setting_config, "wan_type", "type", "0")
                elseif changes.network.wan.proto == "pppoe" then
                    _uci_real:set(pure_basic_setting_config, "wan_type", "type", "1")
                else
                    _uci_real:set(pure_basic_setting_config, "wan_type", "type", "2")
                end
            end
            if changes.network.wan.username then _uci_real:set(pure_basic_setting_config, "wan_type", "pppname", changes.network.wan.username) end
            if changes.network.wan.password then _uci_real:set(pure_basic_setting_config, "wan_type", "pppwd", changes.network.wan.password) end
            if changes.network.wan.ipaddr then _uci_real:set(pure_basic_setting_config, "wan_type", "ip", changes.network.wan.ipaddr) end
            if changes.network.wan.netmask then _uci_real:set(pure_basic_setting_config, "wan_type", "netmask", changes.network.wan.netmask) end
            if changes.network.wan.gateway then _uci_real:set(pure_basic_setting_config, "wan_type", "gateway", changes.network.wan.gateway) end
            if( _uci_real:get("network","wan","dns") ) then
                _uci_real:set(pure_basic_setting_config, "wan_type", "autodns", "0")
                local dns = _uci_real:get("network","wan","dns")
                local dns1, dns2 = dns:match('([^%s]+)%s+([^%s]+)')
                if(not dns1) then
                    _uci_real:set(pure_basic_setting_config, "wan_type", "dns1", dns)
                else
                    _uci_real:set(pure_basic_setting_config, "wan_type", "dns1", dns1)
                    _uci_real:set(pure_basic_setting_config, "wan_type", "dns2", dns2)
                end
            else
                _uci_real:set(pure_basic_setting_config, "wan_type", "autodns", "1")
            end
...
function Map.parse(self, readinput, ...)
    ...
    --add by franklin@siflower, 2015/07/25
    --because the config changes has been commit, so should send to syncservice
	local changes = self.uci:changes()
    --add end
	...
	if self.save then
		...
		if self:submitstate() and ((not self.proceed and self.flow.autoapply) or luci.http.formvalue("cbi.apply")) then
            ...
            --add by kevin@siflower, 2015/09/28 --fetch the valuable changes to the uci
            changes_fetch(changes)
            changes = _uci_real:changes()
            _uci_real:save("wifi_info")
            _uci_real:commit("wifi_info")
	        ...
	        if((changes ~= nil) and (type(changes) == "table") and (next(changes) ~= nil)) then
                -- use socket to send the changes to siwifi's observer service
                local json_encoder = json.Encoder(changes, 4096)
                local json_source = json_encoder:source()
                send_changes(json_source())
	        end
            --add end
		end
		for i, config in ipairs(self.parsechain) do
...
function Map.set(self, section, option, value)
	nixio.syslog("crit", "section = " .. tostring(section) .. ", option = " .. tostring(option) .. ", value = " .. tostring(value))
