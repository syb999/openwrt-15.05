...
function syslog()
	return luci.util.exec("logread | grep daemon")
end
...
function user.setpasswd(username, password)
    ...
    -- local ucil = require "luci.model.uci".cursor()
	-- -- ucil:set("siwifi","key",password)
	-- -- ucil:save("siwifi")
	-- -- ucil:commit("siwifi")
	--
	--echo admin | openssl rsautl -inkey ~/se_pub.key -pubin -encrypt  >info.txt
	os.execute(
		"echo ".. password .. " |  openssl rsautl -inkey /etc/se_pub.key -pubin -encrypt > /etc/info.txt"
	)
