...
#define DB_DEV_FILE DB_PATH "/dev/%s"
...
static int update_devstat(
    const char *dev_ip, uint32_t dev_rxb, uint32_t dev_rxp, uint32_t dev_txb, uint32_t dev_txp
) {
    char path[1024];
    ...
    snprintf(path, sizeof(path), DB_DEV_FILE, dev_ip);

    if (stat(path, &s))
    {
        if (init_file(path, sizeof(struct dev_entry)))
        {
    ...
    e.dev_time = htonl(time(NULL));
    e.dev_rxb  = htonl(dev_rxb);
    e.dev_rxp  = htonl(dev_rxp);
    e.dev_txb  = htonl(dev_txb);
    e.dev_txp  = htonl(dev_txp);

    return update_file(path, &e, sizeof(struct dev_entry));
}
...
{
	FILE *info;
    uint32_t dev_rxb, dev_txb, dev_rxp, dev_txp;
	...
	for (reset_countdown(0); countdown >= 0; countdown--)
	{
        ...
        system("speed run");
		if ((info = fopen("/tmp/dev_speed", "r")) != NULL)
		{
			while (fgets(line, sizeof(line), info))
			{
				if (sscanf(line, DEV_SCAN_PATTERN, dev_ip, &dev_rxb, &dev_rxp, &dev_txb, &dev_txp))
				{
						update_devstat(dev_ip, dev_rxb, dev_rxp, dev_txb, dev_txp);
				}
			}
...
static int run_dump_dev(const char *dev_ip)
{
    int i;
    char path[1024];
    struct file_map m;
    ...
    if (mmap_file(path, sizeof(struct dev_entry), &m))
    {   
        fprintf(stderr, "Failed to open %s: %s\n", path, strerror(errno));
        return 1;
    }   

    for (i = 0; i < m.size; i += sizeof(struct dev_entry))
    {   
        e = (struct dev_entry *) &m.mmap[i];
        ...
        printf("[ %u, %u, %" PRIu32
               ", %u, %u ]%s\n",
            ntohl(e->dev_time),
            ntohl(e->dev_rxb), ntohl(e->dev_rxp),
            ntohl(e->dev_txb), ntohl(e->dev_txp),
            ((i + sizeof(struct dev_entry)) < m.size) ? "," : "");
    }   
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "t:d:i:r:cl")) > -1)
	{
		switch (opt)
		{
			case 't':
