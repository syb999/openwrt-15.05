#!/bin/sh

uci get autodl.@autodl[0].bookurl > /tmp/autodl.bookurl
uci get autodl.@autodl[0].bookname > /tmp/autodl.bookname
uci get autodl.@autodl[0].bookpath > /tmp/autodl.bookpath

autodlbookgeturl=$(cat /tmp/autodl.bookurl)
autodlbookgetname=$(cat /tmp/autodl.bookname)
autodlbookgetpath=$(cat /tmp/autodl.bookpath)

autodlbookcount=1

if [ ! -d "/autodl" ]; then
	mkdir /autodl
	chmod 777 /autodl
	if [ ! -d "/autodl/$autodlbookgetpath" ];then
		ln -s $autodlbookgetpath /autodl
	fi
else
	if [ ! -d "/autodl/$autodlbookgetpath" ];then
		ln -s $autodlbookgetpath /autodl
	fi
fi

cd $autodlbookgetpath

curl -s --retry 3 --retry-delay 2 --connect-timeout 10 -m 20 $autodlbookgeturl | grep \<dd\> > /tmp/autodltmp.zh.tmpbook
while [ ! -s /tmp/autodltmp.zh.tmpbook ]
do
	curl -s --retry 3 --retry-delay 2 --connect-timeout 10 -m 20 $autodlbookgeturl > /tmp/autodltmp.zh.tmpbook
done
cat /tmp/autodltmp.zh.tmpbook | sed '1d' | cut -d "'" -f 2 | cut -d '"' -f 2 > /tmp/autodl.zh.bookurllist
cat /tmp/autodltmp.zh.tmpbook | sed '1d' | cut -d ">" -f 3 | sed 's/<\/a//g' > /tmp/autodl.zh.booknamelist

autodlbooklist=/tmp/autodl.zh.bookurllist

while read LINE
do
	autodlgetbook=$(echo $LINE)
	curl -s --retry 3 --retry-delay 2 --connect-timeout 10 -m 20 $autodlgetbook > /tmp/autodltmp.zh.realbook0
	cat /tmp/autodltmp.zh.realbook0 | grep \<p\> | sed -e 's/<div\ id=\"content\">//g;s/<p>//g;s/<\/p>/\r\n/g' > /tmp/autodltmp.zh.realbook1
	autodlechoname=$(cat /tmp/autodl.zh.booknamelist | head -n 1)
	if [ $autodlbookcount -le 9 ];then
		nautodlbookcount=000$autodlbookcount
		cat /tmp/autodltmp.zh.realbook1 | sed -e "1i$autodlechoname" | sed 's/\t//g' > $autodlbookgetname$nautodlbookcount.txt
	elif [ $autodlbookcount -le 99 ];then
		nautodlbookcount=00$autodlbookcount
		cat /tmp/autodltmp.zh.realbook1 | sed -e "1i$autodlechoname" | sed 's/\t//g' > $autodlbookgetname$nautodlbookcount.txt
	elif [ $autodlbookcount -le 999 ];then
		nautodlbookcount=0$autodlbookcount
		cat /tmp/autodltmp.zh.realbook1 | sed -e "1i$autodlechoname" | sed 's/\t//g' > $autodlbookgetname$nautodlbookcount.txt
	else
		cat /tmp/autodltmp.zh.realbook1 | sed -e "1i$autodlechoname" | sed 's/\t//g' > $autodlbookgetname$autodlbookcount.txt
	fi
	sed 1d -i /tmp/autodl.zh.booknamelist
	autodlbookcount=$(expr $autodlbookcount + 1)
	rm /tmp/autodltmp.zh.realbook0
	rm /tmp/autodltmp.zh.realbook1
done < $autodlbooklist

if [ ! -d "/$autodlbookgetpath/$autodlbookgetname" ]; then
	mkdir $autodlbookgetname
fi

mv -f *.txt $autodlbookgetname

rm /tmp/autodl.*
rm /tmp/autodltmp.*
