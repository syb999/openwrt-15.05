...
first_type() {
	local path_name=${1}
	type -t -p "/bin/${path_name}" -p "${TMP_BIN_PATH}/${path_name}" -p "${path_name}" -p "/usr/bin/{path_name}" "$@" | head -n1
}
...
run_socks() {
	...
	v2ray)
		...
		ln_start_bin "$(first_type $(config_t_get global_app v2ray_file notset) v2ray)" v2ray -config="$config_file"
	;;
	trojan-go)
